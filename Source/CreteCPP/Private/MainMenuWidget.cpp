...
UMainMenuWidget::UMainMenuWidget(const FObjectInitializer& ObjectInitiailizer) : Super(ObjectInitiailizer)
{
...
void UMainMenuWidget::NativeConstruct()
{
	...
	MatchmakingDelegate.BindUFunction(this, "OnMatchmackingButtonClicked");
	...
	if (GameInstance != nullptr)
	{
		...
		{
			AccessToken = CreteGameInstance->AccessToken;
		}
	}
...
void UMainMenuWidget::OnMatchmackingButtonClicked()
{
	...
	if (GameInstance != nullptr)
	{
		...
		{
			AccessToken = CreteGameInstance->AccessToken;
			MatchmakingTicketId = CreteGameInstance->MatchmakingTicketId;
		}
	}
	...
	if (SearchingForGame) {
		...
		if (AccessToken.Len() > 0)
		{
			...
			if (FJsonSerializer::Serialize(RequestObj.ToSharedRef(), Writer)) {
				...
				StartMatchmakingRequest->OnProcessRequestComplete().BindUObject(this, &UMainMenuWidget::OnStartMachmakingResponseReceived);
			...
			if (JsonObject->HasField("access_token") && JsonObject->HasField("id_token") && JsonObject->HasField("refresh_token"))
			{
				...
				if (GameInstance != nullptr)
				{
					...
					if (CreteGameInstance != nullptr)
					{
						...
						CreteGameInstance->SetCognitoTokens(AccessToken, IdToken, RefreshToken);
...
void UMainMenuWidget::OnStartMachmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	if (bWasSuccessful)
	{
		TSharedPtr<FJsonObject> JsonObject;
		...
		if (FJsonSerializer::Deserialize(Reader, JsonObject)) {
			if (JsonObject->HasField("ticketId")) {
				...
				if (GameInstance != nullptr) {
					UCreteGameInstance* CreteGameInstance = Cast<UCreteGameInstance>(GameInstance);
					if (CreteGameInstance != nullptr) {
						CreteGameInstance->MatchmakingTicketId = MatchmakingTicketId;
...
void UMainMenuWidget::OnStopMatchmakingResponseReceived(FHttpRequestPtr Request, FHttpResponsePtr Response, bool bWasSuccessful)
{
	...
	if (GameInstance != nullptr) {
		UCreteGameInstance* CreteGameInstance = Cast<UCreteGameInstance>(GameInstance);
		if (CreteGameInstance != nullptr) {
			CreteGameInstance->MatchmakingTicketId = "";
		}
	}
